...
function setup() {
	...
	mymap = L.map('checkinMap').setView([0, 0], 1); // latitude, longitude, zoom level
	L.tileLayer(tileUrl, {
...
async function getData() {
	...
	for (let item of data) {
		...
		// 	const root = createP('');
		// 	const dateString = new Date(item.timestamp).toLocaleDateString('en-GB', {
		// 		hour: 'numeric',
		// 		minute: 'numeric',
		// 		second: 'numeric'
		// 	});
		// 	const geo = createDiv('latitude: ' + item.lat + '°, ' + 'longitude: ' + item.lon + '°');
		// 	geo.parent(root);
		// 	const time = createDiv(dateString);
		// 	time.parent(root);
	}
}